...
use EnterMedia\API\Exception\APIException;
use EnterMedia\Constants;
use EnterMedia\Object\ObjectInterface;

/**
 * This Class handles the communication with the EnterMedia APIs.
 *
 * @internal
 */
class Client {
   ...
   * from EnterMedia, but it does not adjust it as the time passes.
   *
   * @return int
   *
   * @internal
  ...
  /**
   * Sends an authorized request to a EnterMedia API endpoint.
   *
   * @param string $method
   *   HTTP method.
  ...
  public function request($method, $api_version, $api_type, $account, $endpoint, $result, $is_array = FALSE, ObjectInterface $post = NULL) {
    ...
    // Read on https://docs.entermedia.com/en/video-cloud/cms-api/getting-started/overview-cms.html
